...
class InputMarkup extends StdObject {
    ...
    private function openWrapper() {
        ...
        if ($this->input->wrappersDisabled() === false) {
            // panel class added and removed wpas-field class
            $output .= '<div id="wpas-'.$id.'"  class="wpas-'
                .$id.' wpas-'.$this->input->getFieldType().'-field data-filter-item panel panel-default">';
        }
        return $output;
    }
    ...
    private function inputLabel() {
        ...
        if ($label) {
            // replced label-conainer class with panel-heading and changed label tag with h4 
            $output .= '<div class="panel-heading">' .
                '<h4 class="panel-title hidden-xs"><a role="button" class="collapsed kk">' . $label . '</a></h4></div>';
        }
        return $output;
    }
    ...
    private function listField($is_checkbox = true) {
        ...
        // replaced field-container class with panel-body
        $output = '<div class="wpas-'.$this->input->getId().'-'.$group_label.' wpas-'.$group_label.' panel-body '.$this->input->getClass().'"><ul class="list-group">';
    ...
    private function buttonInput($type) {
        ...
        $value = reset($values);
        $output = '<input type="'.$type.'" class="wpas-'.$type.' reset_button hidden-xs" value="'.$value.'" '.$this->attributesString().'>';
        return $output;
    }
    ...
    private function listOption($value, $label, $type = 'checkbox') {
        ...
        $name .= ($type == 'checkbox') ? '[]' : '';
        $output = '<'.$element.' class="wpas-'.$id.'-'.$type.'-'.$ctr.'-container wpas-'.$id.'-'.$type.'-container list-group-item">';
        if( $type == 'checkbox' ){
            $output .= '<label class="checkbox-inline">';
        }else if ( $type == 'radio' ){
            $output .= '<label class="checkbox-inline radio">';
        }
        $output .='<input type="'.$type.'" id="wpas-'.$id.'-'.$type.'-'.$ctr.'" class="wpas-'.$id.'-'.$type.' wpas-'.$type.'" name="'.$name.'" value="'.$value.'"';
        if (in_array($value, $this->input->getSelected(), true)) {
            $output .= ' checked="checked"';
        }
        $output .= '>';
        if( $type == 'checkbox' ){
            $output .= '<i class="fa fa-square-o fa-2x"></i>';
            $output .= '<i class="fa fa-check-square-o fa-2x"></i>';
        }else if ( $type == 'radio' ){
            $output .= '<i class="fa fa-circle-o fa-2x"></i>';
            $output .= '<i class="fa fa-dot-circle-o fa-2x"></i>';
        }
