...
function wpas_build_ajax_response(array $post) {
    ...
    if ( isset( $request['fastsole_sorting'] ) ) {
        $request['fastsole_sorting'] = fs_sorting_conditions( $request['fastsole_sorting'] );
    }
 ...
 */
function fs_sorting_conditions( $request ) {
    if ( 'price_low_to_high' == $request ) {
        return [
            'meta_key' => '_sf_price',
            'orderby' => 'meta_value',
            'meta_type' => 'NUMERIC',
    ...
    } elseif ( 'price_high_to_low' == $request ) {
        return [
            'meta_key' => '_sf_price',
            'orderby' => 'meta_value',
            'meta_type' => 'NUMERIC',
    ...
    } elseif ( 'release_date_desc' == $request ) {
        return [
            'meta_key' => '_sf_date',
            'orderby' => 'meta_value',
            'meta_type' => 'NUMERIC',
